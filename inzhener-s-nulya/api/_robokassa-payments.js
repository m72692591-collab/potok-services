...
function pathFor(orderId){
  ...
  return `_orders/robokassa/${key}.json`;
}

export async function getRobokassaPayment(orderId){
...
export async function saveRobokassaPayment(orderId,data){
  const current=await getRobokassaPayment(orderId)||{};
  ...
  await put(pathFor(orderId),JSON.stringify(next),{
    ...
    contentType:'application/json',...blobAuth()
  });
  return next;
}